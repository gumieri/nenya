...
- **Pin store** — In-memory LRU map capped at 512 pins. Each pin records the chosen target, the serving account ID, and a last-seen timestamp; idle pins expire after `sticky_session_ttl_seconds` (default `3600`, max `86400`) and the session re-pins on its next request.
- **Account pinning** — On multi-account providers, target build resolves the pinned account first via `AccountManager.SelectAccountByID` and prefers it over LRU selection for the pinned provider/model entry. Health gates: present in the pool, active, not rate-limited (cooldown), not model-locked, and not billing-exhausted (`BillingTracker.IsExhausted`, checked at the gateway layer). Any miss falls back to the normal LRU path. Single-account (legacy `api_key`) providers are unaffected.
